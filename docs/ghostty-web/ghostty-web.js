...
class W {
  ...
  setKittyFlags(A) {
    this.setOption(d.KITTY_KEYBOARD_FLAGS, A);
  }
  encode(A) {
...
const z = class y {
  ...
  getScrollbackLine(A) {
    ...
    if (B < 0)
      return null;
    const E = [], C = this.memory.buffer, I = new Uint8Array(C, this.viewportBufferPtr, B * y.CELL_SIZE), D = new DataView(C, this.viewportBufferPtr, B * y.CELL_SIZE);
    for (let i = 0; i < B; i++) {
      const o = i * y.CELL_SIZE;
      E.push({
  ...
  parseCellsIntoPool(A, Q) {
    const B = this.memory.buffer, E = new Uint8Array(B, A, Q * y.CELL_SIZE), C = new DataView(B, A, Q * y.CELL_SIZE);
    for (let I = 0; I < Q; I++) {
      const D = I * y.CELL_SIZE, i = this.cellPool[I];
...
class P {
  constructor(A) {
    this.bufferChangeEmitter = new n(), this.terminal = A;
  }
  get active() {
...
class v {
  ...
  getCell(A) {
    if (!(A < 0 || A >= this._length))
      return A >= this.cells.length ? new p(
        {
          codepoint: 0,
          fg_r: 204,
          fg_g: 204,
  ...
  handleKeyDown(A) {
    ...
    try {
      if (this.getModeCallback) {
        ...
        this.encoder.setOption(d.CURSOR_KEY_APPLICATION, o);
      }
...
T.URL_REGEX = /(?:https?:\/\/|mailto:|ftp:\/\/|ssh:\/\/|git:\/\/|tel:|magnet:|gemini:\/\/|gopher:\/\/|news:)[\w\-.~:\/?#@!$&*+,;=%]+/gi;
T.TRAILING_PUNCTUATION = /[.,;!?)\]]+$/;
let AA = T;
const x = {
  foreground: "#d4d4d4",
  background: "#1e1e1e",
...
class QA {
  constructor(A, Q = {}) {
    ...
    this.ctx = B, this.fontSize = Q.fontSize ?? 15, this.fontFamily = Q.fontFamily ?? "monospace", this.cursorStyle = Q.cursorStyle ?? "block", this.cursorBlink = Q.cursorBlink ?? !1, this.theme = { ...x, ...Q.theme }, this.devicePixelRatio = Q.devicePixelRatio ?? window.devicePixelRatio ?? 1, this.palette = [
      this.theme.black,
      this.theme.red,
      this.theme.green,
      this.theme.yellow,
  ...
  render(A, Q = !1, B = 0, E, C = 1) {
    ...
    if (c) {
      for (let e = 0; e < o.rows; e++) {
        ...
        if (I > 0)
          if (e < I && E) {
            const F = h - Math.floor(I) + e;
            G = E.getScrollbackLine(F);
          } else {
            const F = e - Math.floor(I);
            G = A.getLine(F);
          }
        else
          G = A.getLine(e);
  ...
  renderBoxDrawing(A, Q, B, E, C) {
    ...
    const J = Math.round(i), N = Math.round(o), U = Math.round(i + h), e = Math.round(o + s);
    for (const G of c) {
      const F = G.weight === "heavy" ? k : M, q = Math.floor(F / 2);
      G.dir === "right" ? I.fillRect(t, a - q, U - t, F) : G.dir === "left" ? I.fillRect(J, a - q, t - J + F, F) : G.dir === "down" ? I.fillRect(t - q, a, F, e - a) : G.dir === "up" && I.fillRect(t - q, N, F, a - N + F);
    }
    I.restore();
  }
...
O.AUTO_SCROLL_EDGE_SIZE = 30;
O.AUTO_SCROLL_SPEED = 3;
O.AUTO_SCROLL_INTERVAL = 50;
let BA = O;
class gA {
  constructor(A) {
    this.terminal = A;
  ...
  processMouseMove(A) {
    ...
    this.linkDetector.getLinkAt(B, t).then((k) => {
      ...
      if (k !== this.currentHoveredLink && ((J = (c = this.currentHoveredLink) == null ? void 0 : c.hover) == null || J.call(c, !1), this.currentHoveredLink = k, (N = k == null ? void 0 : k.hover) == null || N.call(k, !0), this.element && (this.element.style.cursor = k ? "pointer" : "text"), this.renderer))
        if (k) {
          const e = ((U = this.wasmTerm) == null ? void 0 : U.getScrollbackLength()) || 0, G = this.getViewportY(), F = Math.max(0, Math.floor(G)), q = k.range.start.y - e + F, f = k.range.end.y - e + F;
          q < this.rows && f >= 0 ? this.renderer.setHoveredLinkRange({
