...
class H {
  ...
  isBold() {
    return this.cell.flags & F.BOLD ? 1 : 0;
  }
  isItalic() {
    return this.cell.flags & F.ITALIC ? 1 : 0;
  }
  isUnderline() {
    return this.cell.flags & F.UNDERLINE ? 1 : 0;
  }
  isStrikethrough() {
    return this.cell.flags & F.STRIKETHROUGH ? 1 : 0;
  }
  isBlink() {
    return this.cell.flags & F.BLINK ? 1 : 0;
  }
  isInverse() {
    return this.cell.flags & F.INVERSE ? 1 : 0;
  }
  isInvisible() {
    return this.cell.flags & F.INVISIBLE ? 1 : 0;
  }
  isFaint() {
    return this.cell.flags & F.FAINT ? 1 : 0;
  }
  /**
   ...
   */
  isDim() {
    return (this.cell.flags & F.FAINT) !== 0;
  }
}
const j = {
  // Letters
  ...
  handleKeyDown(A) {
    ...
    try {
      if (this.getModeCallback) {
        ...
        this.encoder.setOption(x.CURSOR_KEY_APPLICATION, o);
      }
  ...
  shouldIgnoreBeforeInput(A) {
    if (!this.lastKeyDownData)
      return !1;
    const B = this.getNow() - this.lastKeyDownTime < q.BEFORE_INPUT_IGNORE_MS && this.lastKeyDownData === A;
  ...
  shouldIgnoreBeforeInputFromComposition(A) {
    if (!this.lastCompositionData)
      return !1;
    const B = this.getNow() - this.lastCompositionTime < q.BEFORE_INPUT_IGNORE_MS && this.lastCompositionData === A;
  ...
  shouldIgnoreCompositionEnd(A) {
    if (!this.lastBeforeInputData)
      return !1;
    const B = this.getNow() - this.lastBeforeInputTime < q.BEFORE_INPUT_IGNORE_MS && this.lastBeforeInputData === A;
  ...
  shouldIgnorePasteEvent(A, Q) {
    if (!this.lastPasteData || this.lastPasteSource === Q)
      return !1;
    const E = this.getNow() - this.lastPasteTime < q.BEFORE_INPUT_IGNORE_MS && this.lastPasteData === A;
...
class $ {
  ...
  provideLinks(A, Q) {
    ...
    for (let I = 0; I < C.length; I++) {
      ...
      if (s) {
        let t = I;
        for (let e = I + 1; e < C.length; e++) {
          const k = C.getCell(e);
          if (!k || k.getHyperlinkId() === 0 || this.terminal.wasmTerm.getHyperlinkUri(h, e) !== s)
            break;
          t = e;
        }
        for (let e = I; e <= t; e++)
...
const d = class L {
  ...
  provideLinks(A, Q) {
    ...
    let I = L.URL_REGEX.exec(C);
    for (; I !== null; ) {
      let D = I[0];
      const i = I.index;
      let o = I.index + D.length - 1;
      const h = D.replace(L.TRAILING_PUNCTUATION, "");
...
class QA {
  constructor(A, Q = {}) {
    ...
    this.ctx = B, this.fontSize = Q.fontSize ?? 15, this.fontFamily = Q.fontFamily ?? "monospace", this.cursorStyle = Q.cursorStyle ?? "block", this.cursorBlink = Q.cursorBlink ?? !1, this.theme = { ...p, ...Q.theme }, this.devicePixelRatio = Q.devicePixelRatio ?? window.devicePixelRatio ?? 1, this.palette = [
      this.theme.black,
      this.theme.red,
      this.theme.green,
      this.theme.yellow,
  ...
  renderCellText(A, Q, B, E) {
    ...
    if (A.flags & F.INVISIBLE)
      return;
    const i = this.isInSelection(Q, B);
    let o = "";
    A.flags & F.ITALIC && (o += "italic "), A.flags & F.BOLD && (o += "bold "), this.ctx.font = this.buildFontString(o);
    let h = A.fg_r, s = A.fg_g, t = A.fg_b;
    if (A.flags & F.INVERSE && (h = A.bg_r, s = A.bg_g, t = A.bg_b), E)
