...
class R {
  ...
  createTerminal(A = 80, Q = 24, B) {
    return new Z(this.exports, this.memory, A, Q, B);
  }
  static async load(A) {
    if (A)
      return R.loadFromPath(A);
...
class W {
  ...
  setKittyFlags(A) {
    this.setOption(T.KITTY_KEYBOARD_FLAGS, A);
  }
  encode(A) {
...
const z = class q {
   ...
   */
  getViewport() {
    const A = this._cols * this._rows, Q = A * q.CELL_SIZE;
   ...
   */
  getScrollbackLine(A) {
    const Q = this._cols * q.CELL_SIZE;
    ...
    if (B < 0)
      return null;
    const E = [], C = this.memory.buffer, I = new Uint8Array(C, this.viewportBufferPtr, B * q.CELL_SIZE), D = new DataView(C, this.viewportBufferPtr, B * q.CELL_SIZE);
    for (let i = 0; i < B; i++) {
      const o = i * q.CELL_SIZE;
      E.push({
  ...
  parseCellsIntoPool(A, Q) {
    const B = this.memory.buffer, E = new Uint8Array(B, A, Q * q.CELL_SIZE), C = new DataView(B, A, Q * q.CELL_SIZE);
    for (let I = 0; I < Q; I++) {
      const D = I * q.CELL_SIZE, i = this.cellPool[I];
...
class v {
  ...
  getCell(A) {
    if (!(A < 0 || A >= this._length))
      return A >= this.cells.length ? new x(
        {
          codepoint: 0,
          fg_r: 204,
          fg_g: 204,
...
const j = {
  ...
  extractModifiers(A) {
    let Q = y.NONE;
    return A.shiftKey && (Q |= y.SHIFT), A.ctrlKey && (Q |= y.CTRL), A.altKey && (Q |= y.ALT), A.metaKey && (Q |= y.SUPER), Q;
  }
  /**
  ...
  handleKeyDown(A) {
    ...
    try {
      if (this.getModeCallback) {
        ...
        this.encoder.setOption(T.CURSOR_KEY_APPLICATION, o);
      }
  ...
  shouldIgnoreBeforeInput(A) {
    if (!this.lastKeyDownData)
      return !1;
    const B = this.getNow() - this.lastKeyDownTime < l.BEFORE_INPUT_IGNORE_MS && this.lastKeyDownData === A;
  ...
  shouldIgnoreBeforeInputFromComposition(A) {
    if (!this.lastCompositionData)
      return !1;
    const B = this.getNow() - this.lastCompositionTime < l.BEFORE_INPUT_IGNORE_MS && this.lastCompositionData === A;
  ...
  shouldIgnoreCompositionEnd(A) {
    if (!this.lastBeforeInputData)
      return !1;
    const B = this.getNow() - this.lastBeforeInputTime < l.BEFORE_INPUT_IGNORE_MS && this.lastBeforeInputData === A;
...
const f = class Y {
  ...
  provideLinks(A, Q) {
    ...
    Y.URL_REGEX.lastIndex = 0;
    let I = Y.URL_REGEX.exec(C);
    for (; I !== null; ) {
      let D = I[0];
      const i = I.index;
      let o = I.index + D.length - 1;
      const h = D.replace(Y.TRAILING_PUNCTUATION, "");
...
class QA {
  constructor(A, Q = {}) {
    ...
    this.ctx = B, this.fontSize = Q.fontSize ?? 15, this.fontFamily = Q.fontFamily ?? "monospace", this.cursorStyle = Q.cursorStyle ?? "block", this.cursorBlink = Q.cursorBlink ?? !1, this.theme = { ...d, ...Q.theme }, this.devicePixelRatio = Q.devicePixelRatio ?? window.devicePixelRatio ?? 1, this.palette = [
      this.theme.black,
      this.theme.red,
      this.theme.green,
      this.theme.yellow,
  ...
  render(A, Q = !1, B = 0, E, C = 1) {
    ...
    if (D ? this.currentSelectionCoords = {
      ...
      for (let F = e.startRow; F <= e.endRow; F++)
        M.add(F);
    }
    ...
    const k = /* @__PURE__ */ new Set(), N = this.hoveredHyperlinkId !== this.previousHoveredHyperlinkId, J = JSON.stringify(this.hoveredLinkRange) !== JSON.stringify(this.previousHoveredLinkRange);
    if (N) {
      for (let e = 0; e < o.rows; e++) {
        let F = null;
        if (I > 0)
          if (e < I && E) {
            const G = h - Math.floor(I) + e;
            F = E.getScrollbackLine(G);
          } else {
            const G = e - Math.floor(I);
            F = A.getLine(G);
          }
        else
          F = A.getLine(e);
        if (F) {
          for (const G of F)
            if (G.hyperlink_id === this.hoveredHyperlinkId || G.hyperlink_id === this.previousHoveredHyperlinkId) {
              k.add(e);
              break;
            }
        }
    ...
    for (let e = 0; e < o.rows; e++)
      (I > 0 ? !0 : Q || A.isRowDirty(e) || M.has(e) || k.has(e)) && (c.add(e), e > 0 && c.add(e - 1), e < o.rows - 1 && c.add(e + 1));
    for (let e = 0; e < o.rows; e++) {
      if (!c.has(e))
        continue;
      let F = null;
      if (I > 0)
        if (e < I && E) {
          ...
          const G = I > 0 ? e - Math.floor(I) : e;
          F = A.getLine(G);
        }
      else
        F = A.getLine(e);
      F && this.renderLine(F, e, o.cols);
    }
  ...
  renderCellText(A, Q, B, E) {
    ...
    if (A.flags & r.INVERSE && (h = A.bg_r, s = A.bg_g, t = A.bg_b), E)
      ...
      const c = this.theme.selectionForeground;
      c && c !== "undefined" ? this.ctx.fillStyle = c : this.ctx.fillStyle = this.rgbToCSS(h, s, t);
    } else
    ...
    const N = A.codepoint || 32;
    if (this.renderBlockChar(N, C, I, D) || (N >= 9472 && N <= 9599 ? this.renderBoxDrawing(N, C, I, D, this.metrics.height) : this.renderPowerlineGlyph(N, C, I, D) || this.ctx.fillText(k, a, M)), A.flags & r.FAINT && (this.ctx.globalAlpha = 1), A.flags & r.UNDERLINE) {
      const c = I + this.metrics.baseline + 2;
      this.ctx.strokeStyle = this.ctx.fillStyle, this.ctx.lineWidth = 1, this.ctx.beginPath(), this.ctx.moveTo(C, c), this.ctx.lineTo(C + D, c), this.ctx.stroke();
    }
    if (A.flags & r.STRIKETHROUGH) {
      const c = I + this.metrics.height / 2;
      this.ctx.strokeStyle = this.ctx.fillStyle, this.ctx.lineWidth = 1, this.ctx.beginPath(), this.ctx.moveTo(C, c), this.ctx.lineTo(C + D, c), this.ctx.stroke();
    }
    ...
    if (this.hoveredLinkRange) {
      const c = this.hoveredLinkRange;
      if (B === c.startY && Q >= c.startX && (B < c.endY || Q <= c.endX) || B > c.startY && B < c.endY || B === c.endY && Q <= c.endX && (B > c.startY || Q >= c.startX)) {
  ...
  renderBoxDrawing(A, Q, B, E, C) {
    ...
    const t = Math.round(Q), a = Math.round(B), M = Math.round(Q + E), k = Math.round(B + C), N = new Set(s.map((G) => G.dir)), J = N.has("left"), c = N.has("right"), U = N.has("up"), e = N.has("down"), F = s.some((G) => G.weight === "heavy") ? h : o;
    if (J && c) {
      const G = F, n = Math.floor(G / 2);
      I.fillRect(t, i - n, M - t, G);
    } else
      for (const G of s) {
        if (G.dir !== "left" && G.dir !== "right")
          continue;
        const n = G.weight === "heavy" ? h : o, L = Math.floor(n / 2);
        G.dir === "right" ? I.fillRect(D, i - L, M - D, n) : I.fillRect(t, i - L, D - t, n);
      }
    if (U && e) {
      const G = F, n = Math.floor(G / 2);
      I.fillRect(D - n, a, G, k - a);
    } else
      for (const G of s) {
        if (G.dir !== "up" && G.dir !== "down")
          continue;
        const n = G.weight === "heavy" ? h : o, L = Math.floor(n / 2);
        G.dir === "down" ? I.fillRect(D - L, i, n, k - i) : I.fillRect(D - L, a, n, i - a);
      }
  }
...
const H = class S {
  ...
  constructor(A, Q, B, E) {
    this.selectionStart = null, this.selectionEnd = null, this.isSelecting = !1, this.mouseDownTarget = null, this.mouseButtonsPressed = /* @__PURE__ */ new Set(), this.dirtySelectionRows = /* @__PURE__ */ new Set(), this.selectionChangedEmitter = new K(), this.boundMouseUpHandler = null, this.boundContextMenuHandler = null, this.boundClickHandler = null, this.boundDocumentMouseMoveHandler = null, this.autoScrollInterval = null, this.autoScrollDirection = 0, this.terminal = A, this.renderer = Q, this.wasmTerm = B, this.textarea = E, this.attachEventListeners();
  }
        ...
        if (M && M.codepoint !== 0) {
          ...
          if (M.grapheme_len > 0)
            if (D < C)
              ...
              const N = D - C;
              k = this.wasmTerm.getGraphemeString(N, a);
            }
          else
...
`);
  ...
  updateAutoScroll(A, Q) {
    const B = S.AUTO_SCROLL_EDGE_SIZE;
  ...
  startAutoScroll(A) {
    this.autoScrollInterval !== null && this.autoScrollDirection === A || (this.stopAutoScroll(), this.autoScrollDirection = A, this.autoScrollInterval = setInterval(() => {
      ...
      const Q = S.AUTO_SCROLL_SPEED * this.autoScrollDirection;
...
H.AUTO_SCROLL_EDGE_SIZE = 30;
H.AUTO_SCROLL_SPEED = 3;
H.AUTO_SCROLL_INTERVAL = 50;
let BA = H;
class gA {
  constructor(A) {
    this.terminal = A;
  ...
  constructor(A = {}) {
    this.unicode = {
      ...
      if (this.hasMouseTracking() && E && this.canvas && this.renderer) {
        ...
        let N = 0;
        B.shiftKey && (N += 4), (B.altKey || B.metaKey) && (N += 8), B.ctrlKey && (N += 16);
        const J = Math.min(Math.abs(Math.round(B.deltaY / 33)), 5), U = (B.deltaY < 0 ? 64 : 65) + N;
        for (let e = 0; e < J; e++)
      ...
      if (I >= s && I <= s + h) {
        ...
        const a = o - t * 2, M = this.rows, k = E + M, N = Math.max(20, M / k * a), J = this.viewportY / E, c = t + (a - N) * (1 - J);
        if (D >= c && D <= c + N)
          ...
          const e = 1 - (D - t) / a, F = Math.round(e * E);
    ...
    }, this.ghostty = A.ghostty ?? iA(), this.linkClickHandler = A.onLinkClick;
    const Q = {
      cols: A.cols ?? 80,
      rows: A.rows ?? 24,
    ...
    }), this.cols = this.options.cols, this.rows = this.options.rows, this.buffer = new P(this), this.snapshotBuffer = new gA(this);
  }
  // ==========================================================================
...
`, Q);
  ...
  processMouseMove(A) {
    ...
    this.linkDetector.getLinkAt(B, t).then((k) => {
      var N, J, c, U;
      if (k !== this.currentHoveredLink && ((J = (N = this.currentHoveredLink) == null ? void 0 : N.hover) == null || J.call(N, !1), this.currentHoveredLink = k, (c = k == null ? void 0 : k.hover) == null || c.call(k, !0), this.element && (this.element.style.cursor = k ? "pointer" : "text"), this.renderer))
        if (k) {
          const e = ((U = this.wasmTerm) == null ? void 0 : U.getScrollbackLength()) || 0, F = this.getViewportY(), G = Math.max(0, Math.floor(F)), n = k.range.start.y - e + G, L = k.range.end.y - e + G;
          n < this.rows && L >= 0 ? this.renderer.setHoveredLinkRange({
...
class wA {
  ...
  proposeDimensions() {
    ...
    if (h === 0 || s === 0)
      return;
    const t = h - i - o - IA, a = s - I - D, M = Math.max(EA, Math.floor(t / B.width)), k = Math.max(CA, Math.floor(a / B.height));
